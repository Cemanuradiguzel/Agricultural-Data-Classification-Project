...
Original file is located at
    https://colab.research.google.com/drive/1AchcCpa7puU_t2yJbuq2M5HuG2agytTv
"""
...
df_bihar=df[df["State_Bihar"]==1].copy()
df_bihar=df_bihar.drop(columns=['State_Bihar','State_UP'])

df_bihar

df_up=df[df["State_UP"]==1].copy()
df_up=df_up.drop(columns=['State_Bihar','State_UP'])

df_up

missing_values = df_bihar.isnull().sum()
...
df_bihar_copy = df_bihar.copy()
df_up_copy = df_up.copy()

ws.append(["Bihar Data"])
for row in dataframe_to_rows(df_bihar_copy, index=False, header=True):
    ws.append(row)
...
ws.append(["UP Data"])
for row in dataframe_to_rows(df_up_copy, index=False, header=True):
    ws.append(row)
...
def evaluate_models(X, y, models, test_size=0.2, random_state=53, feature_count=20, min_score_threshold=0.70):
...
X = df_bihar[['Longitude', 'Latitude','Split1Urea','Split2Urea','Split3Urea']]
y = df_bihar['GrainYield']
...
X = df_bihar.drop(columns=['GrainYield'])
y = df_bihar['GrainYield']
...
X = df_up[['Longitude', 'Latitude','Split1Urea','Split2Urea','Split3Urea']]
y = df_up['GrainYield']
...
X = df_up.drop(columns=['GrainYield'])
y = df_up['GrainYield']
